...
```
scripts/<topic>/<descriptive_name>.py    # the teaching script
...
- New concepts get their own topic folder under `scripts/` (with a mirrored folder under `tests/`) when they don't fit an existing one.
- Empty topic folders are seeded with a placeholder `.gitkeep` file (git doesn't track empty directories otherwise). When you add the first script to one of those folders, delete its `.gitkeep` in `scripts/<topic>/` and `tests/<topic>/` in the same commit — the real files hold the folder in the tree from then on.
...
`pyproject.toml` sets `pythonpath = ["."]`, so tests import scripts as `from scripts.<topic>.<script_name> import ...`.
